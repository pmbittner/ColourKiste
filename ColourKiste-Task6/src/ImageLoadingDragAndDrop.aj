...
aspect ImageLoadingDragAndDrop {
    ...
    private DropTargetAdapter Workspace.dropTargetHandler;
    
    after(Workspace w) : target(w) && execution(public Workspace.new(gui.MainFrame)) {
        w.dropTargetHandler = new ImagePanelDropHandler(w);
    }
}